...
def select_algorithm(data,algorithm):
    ...
    try:
        if algorithm == 'svc':
            best,pre_label, sil_score,model = svc(data)
        elif algorithm == 'mlp':
            best,pre_label, sil_score,model = mlp(data)
        elif algorithm == 'sgd':
            best,pre_label, sil_score,model = sgd(data)
        elif algorithm == 'logistic':
            best,pre_label, sil_score,model = logistic(data)
        elif algorithm == 'linearsvc':
            best,pre_label, sil_score,model = linearsvc(data)
        elif algorithm == 'gaussiann':
            best,pre_label, sil_score,model = gaussiann(data)
        elif algorithm == 'multinomialnb':
            best,pre_label, sil_score,model = multinomialnb(data)
        elif algorithm == 'dt':
            best, pre_label, sil_score, model = dt(data)
        elif algorithm == 'knn':
            best, pre_label, sil_score, model = knn(data)
    except Exception as err:
        return {},[],err,[]
    else:
...
def insert_User(file_name,file_path,user_name,alg,Train1Mf_values,Train1ResultNorm_values):
    ...
    # User_df.to_csv('D:\\superlloy\\automl\\cluster\\selection\\recommend_user\\' + file_name.split('.')[0] + '_userbased.csv')

    date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    file_name = file_name
    user_name = user_name
    User_alg_list = ['knn', 'logi', 'dt']
    type = 'User_Based'
...
def insert_Item(file_name,file_path,user_name,alg,Train1ResultNorm_values,Train1Result_values,cluster_list):
    ...
    #
    # Item_df.to_csv('D:\\gwhj\\superlloy\\automl\\cluster\\selection\\recommend_item\\' + file_name.split(
    #         '.') [0]+ '_itembased.csv')

    date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    file_name = file_name
    user_name = user_name
    Item_alg_list = ['knn', 'dt', 'sgd']
    type = 'Item_Based'
    print('Insert DataBase:')
    db.RecommendResult.insert({
        "date": date,
...
def insert_Model(file_name, file_path, user_name, alg, Train1Result_values, cluster_list):
    ...
    #
    # Model_df.to_csv(
    #     'D:\\gwhj\\superlloy\\automl\\classifier\\selection\\recommend_model\\' + file_name.split('.')
    #     + '_modelbased.csv')

    date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    file_name = file_name
    user_name = user_name
    Model_alg_list = ['knn', 'mlp', 'sgd']
    type = 'Model_Based'
    print('Insert DataBase:')
    db.RecommendResult.insert({
        "date": date,
