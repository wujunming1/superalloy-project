...
def dbscan(data,eps,min_samples,metric):
    ...
    mod = DBSCAN(eps = eps,min_samples = min_samples,metric = metric)
    mod.fit(data)
    pred = mod.fit_predict(data)
    score = sil_score(data,pred)
    print(score)
    return mod,pred,score
...
#插入数据操作
def insert(file_name,file_path,alg,eps,min_samples,metric,username):
    ...
    # model, label, score = dbscan(data,eps,min_samples,metric)
...
if __name__ == '__main__':
    ...
    # insert(file_name,file_path,alg,eps,min_samples,metric,username)
    # print(1)
    ...
    print(data_file)
    insert(parameterlist[0], parameterlist[1], parameterlist[2], parameterlist[3],parameterlist[4],parameterlist[5],parameterlist[6])
