...
##SVM
def SVM(data,svm_c,svm_kernel):
    x = data[:,:-1]
    y = data[:,-1]
    x_train, x_test, y_train, y_test = train_test_split (x, y, test_size = 0.3, random_state = 0)
    model = svm.SVR(C=svm_c,kernel=svm_kernel)
...
#插入数据操作
def insert(file_name,file_path,alg,svm_c,svm_kernel,username):
    ...
    svm_c=float(svm_c)
    try:
        model, label, y_test, score, xy = SVM(data,svm_c,svm_kernel)
