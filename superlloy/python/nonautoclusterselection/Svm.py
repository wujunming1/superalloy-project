...
#Svm
def svm(X_train,y_train,svm_c,svm_gamma,svm_degree,svm_kernel):
    # metric_list = ['euclidean', 'manhattan', 'chebyshev']
    # ap = AffinityPropagation(damping = args['damping'])
    def RBFKernelSVC(svm_c,svm_gamma,svm_degree,svm_kernel):  # 返回归一化的svc分类器
                        return Pipeline([
                            ("std_scaler", StandardScaler()),
                            ("svc", SVC())
                        ])
    db = RBFKernelSVC(svm_c,svm_gamma,svm_degree,svm_kernel).fit(X_train, y_train)
...
#插入数据操作
def insert(file_name,file_path,alg,svm_c,svm_gamma,svm_degree,svm_kernel,username):
    ...
    try:
        model, label, score = svm(X_train,y_train,svm_c,svm_gamma,svm_degree,svm_kernel)
    except Exception as err:
        print(err)
    y_score = model.fit(X_train, y_train).decision_function(X_train)
...
if __name__ == '__main__':
    ...
    insert(parameterlist[0], parameterlist[1], parameterlist[2], parameterlist[3],parameterlist[4],parameterlist[5],parameterlist[6],parameterlist[7])

    # data_file = load_data(r'D:\gwhj\superlloy\data\piki\iris.xlsx')
    # print("adasdsadasdadad")
    # insert('iris.xlsx', 'D:\gwhj\superlloy\data\piki\iris.xlsx', 'svm', '0.2', '0.2', '2', 'rbf', 'piki')
