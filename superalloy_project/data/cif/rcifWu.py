...
def read_file(path, cif):
    ...
    if (not(os.path.isdir(path)) and (".cif" in path)):
        try:
            atoms=ase.io.read(path,store_tags=True)
            ...
            print("atom info:",atoms.info)
            # print(atoms.info["_audit_update_record"])
            ...
            print("atom info origin", atoms.info[origin])
        ...
        for index, file in enumerate(files):
            ...
            if (not(os.path.isdir(filePath)) and (".cif" in filePath)):        
                try:
                    atoms = ase.io.read(filePath,store_tags=True)
                except Exception as errorInfo:
                    if (type(errorInfo) == UserWarning):
                        try:
                            f = open("warningLog", "a")
...
def get_info(filename, atoms, origin):
    ...
    if ('_audit_creation_date' in atoms.info):
        print(atoms.info['_audit_creation_date'])
        a_creation_date=atoms.info["_audit_creation_date"]
        # create_time=a_creation_date[0:8]
        # print("创建时间：",create_time)
        # list(str(a_creation_date).split("/")).in
        Create_date['_audit_creation_date'] = atoms.info["_audit_creation_date"]
    if ('_audit_update_record' in atoms.info):
        Create_date['_audit_update_record'] = atoms.info['_audit_update_record']
    dict['create_date'] = Create_date
    return dict
db = get_db();
cif = get_collection(db)
#read_file(sys.argv[1], cif)
# read_file('icsd_000007.cif', cif)
parameterlist=[];
