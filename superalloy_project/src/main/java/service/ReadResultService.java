...
//                            System.out.println(i + "行" + j + " 列 is default type");
    ...
    public Map<Object, Object> ExcelConvertJSONString(ArrayList<ArrayList<Object>> list){
...
//				 document.append("")
                for(int j=0;j<title.size();j++){
                    try{
                        json.put(title.get(j).toString(), list.get(i).get(j));
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                }
            ...
            objectMap.put("clusterData", jsonArray.toJSONString());
            objectMap.put("headTitle", JSONObject.toJSONString(title));
            return objectMap;
