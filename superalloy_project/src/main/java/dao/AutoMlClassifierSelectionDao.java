...
@Repository
public class AutoMlClassifierSelectionDao {
    @Autowired
    ...
    //清楚对应用户、文件、算法的分类模型，用于更新
    public void ClassifierModel_Clean(String username,String filename,String alg){
        mongo.remove(Query.query(Criteria.where("user_name").is(username).
                and("file_name").is(filename).and("algorithm_name").is(alg)),Document.class,"ClassifierModel");
    }

    //取出对应用户、文件、算法的分类模型
    public ClassifierModelEntity ClassifierModel_getResult(String username,String filename,String alg){
        List<ClassifierModelEntity> lists = mongo.find(Query.query(Criteria.where("user_name").is(username).
                and("file_name").is(filename).and("algorithm_name").is(alg)),ClassifierModelEntity.class,"ClassifierModel");
        return lists.get(0);
    }
